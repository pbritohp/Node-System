...
if (isset($_GET['delete_id'])) {
    $id = $_GET['delete_id'];
    $sql = "DELETE FROM planStr WHERE id = ?";
...
if (isset($_POST['submit'])) {
    ...
    $query = mysqli_prepare($link, "INSERT INTO planStr(nome, idStr, idInf, Descr, idPlan, dataIn) VALUES (?, ?, ?, ?, ?, NOW())");
    mysqli_stmt_bind_param($query, "siisi", $nome, $idStr, $idInf, $comment, $id);
...
if (isset($_POST['editSubmit'])) {
    ...
    $query = mysqli_prepare($link, "UPDATE planStr SET nome=?, idInf=?, Descr=? WHERE id=?");
    mysqli_stmt_bind_param($query, "sisi", $nome, $idInf, $comment, $id);
    if (mysqli_stmt_execute($query)) {
        $successSave = "Edição realizada com sucesso!";
        header("Location: planStr.php?id=" . $_GET['id'] . "&success=1");
        exit();
    } else {
...
<body> 
    ...
    <div class="sidebar">
        <div class="titleFat">
                    ...
                    <h3 class = "nomePage">Estratégia</h3>    
            </div>
            ...
            <a href="export_table_csv.php?table=planstr&id=<?php echo $_GET['id']; ?>" class="a_home">Exportar Tabela para CSV</a>
            <a href="export_table_xml.php?table=planstr&id=<?php echo $_GET['id']; ?>" class="a_home">Exportar Tabela para XML</a>
        </div>
        <div class="content-box">
            <?php
            $queryTipo = "SELECT * FROM typeFat WHERE id > 16";
            ...
            while ($rowTipo = mysqli_fetch_assoc($resultTipo)) {
                ...
                $queryFat = "SELECT planStr.idInf, planStr.Descr, planStr.id, planStr.nome, typeInf.nome AS influencia_nome
                    FROM planStr INNER JOIN typeInf ON planStr.idInf = typeInf.id WHERE idStr = $idTipo AND idPlan = $id";
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            newPlanModalLinks.forEach(function(link) {
                link.onclick = function() {
                    ...
                    document.getElementById('idStr').value = typeId;
            ...
            $('.delete-factor-button').click(function() {
                ...
                if (confirm('Você tem certeza que deseja excluir este fator?')) {
                    window.location.href = "planStr.php?id=<?php echo $_GET['id']; ?>&delete_id=" + id;
                }
            });
