...
if (isset($_POST['submit'])) {
    ...
    $queryCheck = mysqli_prepare($link, "SELECT id FROM typeSWOT WHERE id = ?");
    ...
    if (mysqli_num_rows($resultCheck) > 0) {
        $query = mysqli_prepare($link, "INSERT INTO SWOT (nome, idTS, Descr, idPlan) VALUES (?, ?, ?, ?)");
...
if (isset($_POST['editSubmit'])) {
    ...
    $query = mysqli_prepare($link, "UPDATE SWOT SET nome=?, Descr=? WHERE id=?");
    mysqli_stmt_bind_param($query, "ssi", $nome, $comment, $id);
    if (mysqli_stmt_execute($query)) {
        $successSave = "Edição realizada com sucesso!";
        header("Location: swot.php?id=" . $_GET['id'] . "&success=1");
        exit();
    } else {
...
<?php include('nav.php');?>
    ...
    <div class="plans">
        ...
        <div class="titleFat">
            ...
            <a href="export_table_csv.php?table=SWOT&id=<?php echo $_GET['id']; ?>" class="a_home">Exportar Tabela para CSV</a>
            <a href="export_table_xml.php?table=SWOT&id=<?php echo $_GET['id']; ?>" class="a_home">Exportar Tabela para XML</a>
        </div>
        <div class="content-box">
            <?php
            $queryTipo = "SELECT * FROM typeSWOT";
            ...
            while ($rowTipo = mysqli_fetch_assoc($resultTipo)) {
                ...
                $queryFat = "SELECT * FROM SWOT WHERE idTS = $idTipo AND idPlan = $id";
           ...
           // Evento ao clicar no botão de edição
            ...
            $('.delete-factor-button').click(function() {
                ...
                if (confirm('Você tem certeza que deseja excluir este fator?')) {
                    window.location.href = "swot.php?id=<?php echo $_GET['id']; ?>&delete_id=" + id;
                }
            });
